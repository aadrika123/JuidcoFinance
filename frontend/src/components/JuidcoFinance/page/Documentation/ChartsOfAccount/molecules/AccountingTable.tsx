      ...
      <tr
        ...
        className={
          ...
          (!d
            ? ""
            : d.code_type == 1
              ? "bg-primary_bg_indigo text-white bg-opacity-80"
              : d.code_type == 2
                ? "bg-primary_bg_indigo bg-opacity-25 text-slate-950"
                : "text-secondary")
        }
      >
        {/* MAJOR HEAD */}
          ...
          {props.searchCondition
            ? parse(
                d?.description.replaceAll(
                  props.searchCondition,
                  '<span className="bg-[#FFCD00] text-white">$1</span>'
                )
              )
            : d?.description}
        </td>
        ...
        <td className="border border-zinc-300 ">
          {fc(d?.balance < 0 ? -d.balance : 0)}
        </td>
        <td className="border border-zinc-300 ">
          {fc(d?.balance >= 0 ? d.balance : 0)}
        </td>
        <td className="border border-zinc-300">
          <Button variant="primary" onClick={() => props.onViewButtonClick(d)}>
            View
          </Button>
        </td>
      </tr>
            ...
            <table className="table table-md">
              <thead className=" text-white text-[1rem] sticky top-0">
                <tr className="bg-white text-center">
                  ...
                  <th
                    className="border text-secondary_black border-zinc-400 font-medium p-0 bg-primary_bg_gray w-[35%]"
                    rowSpan={2}
                  >
                    <div className="text-center bg-primary_bg_gray p-4">
              ...
              <tbody className="static">{rows ? rows : <></>}</tbody>
            </table>
          </div>
        </motion.div>
      )}
