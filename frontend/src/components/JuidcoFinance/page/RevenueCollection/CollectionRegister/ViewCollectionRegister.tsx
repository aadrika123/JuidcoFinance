...
export const ViewCollectionRegister = ({
  CollectionRegisterID,
}: {
  CollectionRegisterID: string;
}) => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios({
        method: "GET",
        url: `${FINANCE_URL.COLLECTION_REGISTER.getById}/${CollectionRegisterID}`,
      });
  ...
  const { refetch: reloadData, isFetching: isFetching } = useQuery(
    ["collection-register-get-single", CollectionRegisterID],
    fetchData
  );
