        ...
        <>
            ...
            {showAddingForm && (
                <Popup title="View Bill" zindex={10} width={60}>
                    <BillEntryFormComponent onSubmit={addNewRecord} onClose={() => setShowAddingForm(false)}/>
                </Popup>
            )}


            <section className="mt-8 border-b border-r border-l">
                <div className="flex items-center"><SubHeading className="text-2xl">Bills</SubHeading></div>

                <SimpleTable columns={columns} data={data} onViewButtonClick={onViewButtonClick} />

                <div className='flex justify-center'><Button variant="cancel" onClick={() => setShowAddingForm(true)}>Add New Bill Entry + </Button></div>
            </section>

            <section className="mt-8 border-b border-r border-l">
                <Button variant="primary" onClick={submitAll}>Submit</Button>
            </section>
